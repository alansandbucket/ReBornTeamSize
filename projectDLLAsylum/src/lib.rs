...
unsafe fn on_level_start_callback(){
    ...
    if (CONFIG_GLOBAL.clone().unwrap().inPVP == "no".to_string())
    {
...
fn main_thread() {
    let map_ipc_dict_pve: HashMap<&str, Vec<u16>> = HashMap::from([
        ...
        ("IncTut_Freeze_P", wchz!("open IncTut_Freeze_P").to_vec()),
        ("Dojo_P", wchz!("open Dojo_P").to_vec()),
    ...
    let map_ipc_dict_pvp: HashMap<&str, &str> = HashMap::from([
		...
		("Snowdrift_P", "open Snowdrift_P?bTournamentMode=1?SpawnBotsTeamA?SpawnBotsTeamB"),
		("Snowblind_Headhunter_P", "open Snowblind_Headhunter_P?bTournamentMode=1?SpawnBotsTeamA?SpawnBotsTeamB"),
    ...
    unsafe{
       ...
       // ORIG_STATICCREATEOBJECT_ADDR = MinHook::create_hook(static_construct_object as _, fake_static_construct_object as _).unwrap() as usize;
        ...
        if map_ipc_dict_pve.contains_key(&config.mapToLoad as &str)
        {
            let command_1 = map_ipc_dict_pve[&config.mapToLoad as &str].as_slice();
            engine_call_command(_uobjects[0].address + 0x25ebde8, ptr::addr_of!(*command_1) as *const () as usize, 0);
        }
        else
        {
            let mut commandStr = map_ipc_dict_pvp[&config.mapToLoad as &str].to_string();
            commandStr = commandStr.replace("SpawnBotsTeamA", format!("{}{}", "SpawnBotsTeamA=", &config.teamSizeA).as_str());
            commandStr = commandStr.replace("SpawnBotsTeamB", format!("{}{}", "SpawnBotsTeamB=", &config.teamSizeB).as_str());
            let commandVec: Vec<u16> = commandStr.encode_utf16().collect();
            let commandSlice = commandVec.as_slice();
            //let command_1 = map_ipc_dict[&config.mapToLoad as &str].as_slice();
            engine_call_command(_uobjects[0].address + 0x25ebde8, ptr::addr_of!(*commandSlice) as *const () as usize, 0);
        }
        loop{
